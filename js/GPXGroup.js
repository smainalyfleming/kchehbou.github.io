...
L.GpxGroup = L.Class.extend({
 ...
 _onRouteAddLine: function(route, e) {
  var polyline = e.line;
  route.on("selected",
   L.bind(this._onRouteSelected, this, route, polyline));
 ...
 highlight: function(route, polyline) {
  ...
  if (this.options.distanceMarkers) polyline.addDistanceMarkers()
 },
 ...
 _onRouteLoaded: function(route) {
  if (this.options.legend) this._legend.addBaseLayer(route, '<svg id="legend_' + route._leaflet_id + '" width="25" height="10" version="1.1" xmlns="http://www.w3.org/2000/svg">' + '<line x1="0" x2="50" y1="5" y2="5" stroke="' + route.options.polyline_options.color +
   '" fill="transparent" stroke-width="5" /></svg>' + " " + route.get_name());
