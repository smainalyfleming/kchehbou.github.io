L.Control.Elevation = L.Control.extend({
 ...
 addData: function(d, layer) {
  ...
  if ((typeof layer ===
    "undefined" || layer === null) && d.on) layer = d;
  if (layer) {
 ...
 addTo: function(map) {
  if (this.options.detachedView) this._addToChartDiv(map);
 ...
 clear: function() {
  ...
  if (this._map) this._map.fireEvent("eledata_clear")
 },
 ...
 onAdd: function(map) {
  ...
  this._applyData();
  this._map.on("zoom viewreset zoomanim",
   this._hidePositionMarker, this);
 ...
 _addData: function(d) {
  var geom =
   d && d.geometry && d.geometry;
  var i;
  if (geom) switch (geom.type) {
   case "LineString":
  ...
  if (feat)
   for (i = 0; i < d.features.length; i++) this._addData(d.features[i]);
  if (d && d._latlngs) this._addGPXdata(d._latlngs)
 },
 _addGeoJSONData: function(coords) {
  if (coords)
   for (var i =
     0; i < coords.length; i++) this._addPoint(coords[i][1], coords[i][0], coords[i][2])
 },
 _addGPXdata: function(coords) {
  if (coords)
   for (var i = 0; i < coords.length; i++) this._addPoint(coords[i].lat, coords[i].lng, coords[i].meta.ele)
 },
 _addPoint: function(x, y, z) {
  ...
  var delta = curr.distanceTo(prev) *
 ...
 _appendChart: function(svg) {
  var g =
   svg.append("g").attr("transform", "translate(" + this.options.margins.left + "," + this.options.margins.top + ")");
  this._appendGrid(g);
 ...
 _appendXaxis: function(axis) {
  axis.append("g").attr("class", "x axis").attr("transform", "translate(0," + this._height() + ")").call(d3.axisBottom().scale(this._x).ticks(this.options.xTicks)).append("text").attr("x", this._width() + 6).attr("y", 30).text(this._xLabel)
 },
 _appendXGrid: function(grid) {
  grid.append("g").attr("class",
   "x grid").attr("transform", "translate(0," + this._height() + ")").call(d3.axisBottom().scale(this._x).ticks(this.options.xTicks).tickSize(-this._height()).tickFormat(""))
 },
 ...
 _appendFocusRect: function(g) {
  ...
  if (L.Browser.mobile) {
   focusRect.on("touchmove.drag", this._dragHandler.bind(this)).on("touchstart.drag",
    this._dragStartHandler.bind(this)).on("touchstart.focus", this._mousemoveHandler.bind(this)).on("touchmove.focus", this._mousemoveHandler.bind(this));
  ...
  L.DomEvent.on(this._container, "mouseup", this._dragEndHandler,
   this)
 },
 ...
 _appendMouseFocusG: function(g) {
  ...
  this._focuslabelrect = focusG.append("rect").attr("class", "mouse-focus-label").attr("x", 0).attr("y", 0).attr("width", 0).attr("height",
   0).attr("rx", 3).attr("ry", 3);
 ...
 _appendPositionMarker: function(pane) {
  ...
  this._mouseHeightFocus = heightG.append("svg:line").attr("class", theme +
   " height-focus line").attr("x2", 0).attr("y2", 0).attr("x1", 0).attr("y1", 0);
  this._pointG = heightG.append("g");
  this._pointG.append("svg:circle").attr("class", theme + " height-focus circle-lower").attr("r", 6).attr("cx", 0).attr("cy", 0);
  this._mouseHeightFocusLabel = heightG.append("svg:text").attr("class", theme + " height-focus-label").style("pointer-events", "none")
 },
 _applyData: function() {
 ...
 _fitSection: function(index1, index2) {
  ...
  var ext = this._calculateFullExtent(this._data.slice(start,
   end));
 ...
 _hidePositionMarker: function() {
  if (!this.options.autohidePositionMarker) return;
 ...
 _initChart: function() {
  ...
  if (opts.responsiveView)
   if (opts.detachedView) {
    var offsetWi =
     document.querySelector(opts.elevationDiv).offsetWidth;
  ...
  var y = this._y = d3.scaleLinear().range([this._height(),
   0
  ]);
  var area = this._area = d3.area().curve(opts.interpolation).x(function(d) {
   return d.xDiagCoord = x(d.dist)
  }).y0(this._height()).y1(function(d) {
   return y(d.z)
  });
  var line = this._line = d3.line().x(function(d) {
   return d3.mouse(svg.select("g"))[0]
  }).y(function(d) {
   return this._height()
  });
  var container = this._container;
  var cont = d3.select(container).attr("width", opts.width);
  var svg = cont.append("svg").attr("class", "background").attr("width", opts.width).attr("height", opts.height);
  this._appendChart(svg)
 },
 _initToggle: function(container) {
  container.setAttribute("aria-haspopup",
   true);
  if (!this.options.detachedView) L.DomEvent.disableClickPropagation(container);
 ...
 _isJSONDoc: function(doc, lazy) {
  ...
  if (typeof doc === "string" && lazy) {
   doc = doc.trim();
   return doc.indexOf("{") == 0 || doc.indexOf("[") ==
    0
  } else {
   try {
    doc = doc.toString();
 ...
 _mouseoutHandler: function() {
  if (!this.options.detachedView) this._hidePositionMarker()
 },
 ...
 _showDiagramIndicator: function(item, xCoordinate) {
  ...
  var alt = item.z,
   ...
   numX = opts.hoverNumber.formatter(dist,
  ...
  if (this._isDomVisible(focuslabeltext)) {
   ...
   var padding = 2;
   this._focuslabelrect.attr("x", bbox.x - padding).attr("y", bbox.y - padding).attr("width", bbox.width + padding * 2).attr("height", bbox.height +
    padding * 2);
 ...
 _showPositionMarker: function(item) {
  this._selectedItem =
   item;
  if (this.options.useLeafletMarker) this._updateLeafletMarker(item);
  else this._updatePositionMarker(item)
 },
 ...
 _updateHeightIndicator: function(item) {
  ...
  this._mouseHeightFocusLabel.attr("x", item.x).attr("y", normalizedY).text(numY + " " + this._yLabel).style("visibility", "visible")
 },
 ...
 _updatePositionMarker: function(item) {
  ...
  if (this.options.useMapIndicator) this._updatePointG(layerpoint);
  if (this.options.useHeightIndicator) this._updateHeightIndicator(layerpoint)
 },
 _width: function() {
  var opts = this.options;
